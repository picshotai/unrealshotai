...
// Configure the Caveat font for "Create" text
const caveat = Caveat({
  subsets: ['latin'],
  weight: '400',
});
...
export default function ScribbleAnimation() {
  ...
  useEffect(() => {
    ...
    const typeEffect = () => {
      ...
      if (isDeleting) {
        setText(currentPrompt.substring(0, charIndex - 1));
        charIndex--;
      } else {
        setText(currentPrompt.substring(0, charIndex + 1));
        charIndex++;
  ...
  return (
    <motion.div
      ...
      className="absolute sm:-top-10 -top-9 left-5 sm:left-2 transform sm:text-xl text-sm flex items-center"
    >
      {/* "Create" text with SVG arrow */}
      <div className="absolute -top-16 -left-30 flex flex-col">
        <span
          className="font-light text-2xl text-indigo-600 text-shadow"
          style={{  
        ...
        <svg 
          className="w-12 h-12 mt-2 mb-2 fill-indigo-600 fill-base-content/80 rotate-30" 
      ...
      {/* Animated photo styles text */}
      <span
        className="font-handwritten text-lg font-semibold text-shadow mt-[22px] mb-4" // Adjusted from mt-6 to mt-[22px] (2px upward)
        style={{
